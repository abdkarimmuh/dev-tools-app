...
**UI components**: shadcn/ui primitives live in `components/ui/` (Tailwind v4 + `class-variance-authority`, `@base-ui/react` under the hood — NOT Radix, see below) — prefer composing these over hand-rolling. `components.json` defines the shadcn config (style `base-vega`, base color `neutral`, icon library `lucide`). Two-panel input/output layout (left=input, right=output, `grid lg:grid-cols-2`) is the standard shape for formatter/converter/encoder tools; look at `app/tools/(text)/markdown-preview/page.tsx` or `app/tools/(encoding)/base-encoder/page.tsx` as reference implementations.

**This is Base UI, not Radix — `asChild` does not exist.** Training data and muscle memory both default to Radix's `asChild` prop for rendering a custom element (e.g. a `<Button>` that's actually a `<Link>`). This codebase's primitives are `@base-ui/react`, which uses a `render` prop instead: `<Button render={<Link href="/">Home</Link>} />` rather than `<Button asChild><Link href="/">Home</Link></Button>`. Passing `asChild` is a silent no-op at runtime *and* a `tsc` type error (the prop doesn't exist on any wrapper) — grep `components/ui/*.tsx` for `render` if unsure whether a given primitive supports it. Whole-project migration history/rationale (including preset-resolved `menuColor`/`fontHeading` quirks and one hand-preserved customization in `sidebar.tsx`) is in `.migration/project.md` and one file per component in `.migration/`.
...
| `@base-ui/react`                          | Headless primitives behind `components/ui/` (not Radix — see the `asChild`/`render` note above)                                                                                                                                  |
