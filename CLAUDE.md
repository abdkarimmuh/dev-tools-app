...
Separately, `useStorage(key, defaultValue, "local" | "session")` (`hooks/use-storage.ts`) is real `localStorage`/`sessionStorage` persistence, currently only used for language (`contexts/language-context.tsx`) and theme.
